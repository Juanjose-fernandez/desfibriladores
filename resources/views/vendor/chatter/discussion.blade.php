...
@section('content')
    <div id="chatter" class="discussion row">
        <div class="col-sm-12">
            <div class="well">
                <table class="table table-striped table-froum">
                    <thead>
                    <tr>
                        <th colspan="2">
                            <a class="back_btn" href="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}"><i class="chatter-back"></i></a>
                            <h1>{{ $discussion->title }}</h1><span class="chatter_head_details"> @lang('chatter::messages.discussion.head_details')<a class="chatter_cat" href="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
                        </th>
                    </tr>
                    </thead>
                    <div>
                    ...
                    <tr data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
                        <td class="text-center">
                            <a href="user_profile">
                                <strong>{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</strong>

                            </a>
                        </td>
                        <!-- FECHA Y HORA -->
                        <td style="padding-top: 7px!important">
                            <span class="ago chatter_middle_details">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->format('d-m-Y H:i:s') }}</span>
                        ...
                        <!-- END FECHA Y HORA -->
                            ...
                            @if(!Auth::guest() && (Auth::user()->id == $post->user->id))
                                ...
                                <div id="delete_warning_{{ $post->id }}" class="chatter_warning_delete" style="display:none">
                                ...
                                <div class="chatter_post_actions pull-rigth">
                                    <p class="chatter_delete_btn" style="
                                            margin: 0px;
                                            float: right;
                                            background: #f1fcff;
                                            border-radius: 10px;
...
">
                                    ...
                                    <p class="chatter_edit_btn" style="
                                            margin: 0px;
                                            float: right;
                                            background: #f1fcff;
                                            border-radius: 10px;
...
">
                    ...
                    <tr>
                        ...
                        <td class="text-center" style="width: 12%;padding-top: 50px!important;padding-bottom: 30px!important;">
                            <div class="push-bit" >
                                <div class="chatter_avatar" style="margin-left:3%; margin-top:-40px">
                        ...
                        <td>
                            ...
                            @else
                                <?= $post->body; ?>
                            @endif
                            <em>{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}
                                {{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_surname')}) }}
                                firma usuario</em>
                                </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if(!Auth::guest())
                                <div id="new_response row">

                                    <div id="new_discussion col-md-12">
                                        ...
                                        <form id="chatter_form_editor" action="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/posts" method="POST" style="border:solid 1px black; margin-top:10px; border-radius: 3px ">
        ...
        <div class="container margin-top">
                        ...
                        @else
                            <div class="col-md-3 left-column">
                                ...
                                <div class="chatter_sidebar">
                                    ...
                                    <a href="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i> @lang('chatter::messages.discussion.all')</a>
                                    <ul class="nav nav-pills nav-stacked">
                                        <?php $categories = DevDojo\Chatter\Models\Models::category()->all(); ?>
                                        @foreach($categories as $category)
                                            <li><a href="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.category') }}/{{ $category->slug }}"><div class="chatter-box" style="background-color:{{ $category->color }}"></div> {{ $category->name }}</a></li>
                                        @endforeach
                                    </ul>
                                </div>
                                <!-- END SIDEBAR -->
                                ...
                                @if(!Auth::guest())
		        					...
		        					{{ strtoupper(substr(Auth::user()->{Config::get('chatter.user.database_field_with_user_name')}, 0, 1)) }}
                                        ...
                                        <div id="new_discussion" >
                                            ...
                                            <form id="chatter_form_editor" action="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/posts" method="POST">
            ...
            @if(Config::get('chatter.sidebar_in_discussion_view'))
                <div id="new_discussion_in_discussion_view">
                    ...
                    <form id="chatter_form_editor_in_discussion_view" action="{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}" method="POST">
                        <div class="row">
                            <div class="col-md-7">
                                <!-- TITLE -->
            ...
            <script>
                $('document').ready(function(){
                    ...
                    $('.chatter_edit_btn').click(function(){
                        ...
                        container = parent.next('tr').find('.chatter_middle');
...
initializeNewTrumbowyg('post-edit-' + id);
                    ...
                    $('.cancel_chatter_edit').click(function(){
                        //post_id = $(e.target).data('id');
                        post_id = $(e.target).prev().data('id');
...
$(e.target).parents('li').find('.trumbowyg').fadeOut();
                    ...
                    $('.chatter_delete_btn').click(function(){
                        ...
                        //parent = $(this).parent().siblings('.chatter_warning_delete');
                        parent = $(this).parent().parent().parent();
                        parent.addClass('delete_warning');
                        id = parent.data('id');
                        $('#delete_warning_' + id).show();
                    ...
                    $('.delete_response').click(function(){
                        post_id = $(this).parent().parent().parent().data('id');
                        $.form('{{URL::to('/')}}/{{ Config::get('chatter.routes.home') }}/posts/' + post_id, { _token: '{{ csrf_token() }}', _method: 'DELETE'}, 'POST').submit();
                    });
